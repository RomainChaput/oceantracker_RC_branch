...
class DielVerticalMigration(_VelocityModiferBase):
    ...
    def __init__(self,):
        ...
        self.add_default_params({'vertical_swimming_speed': PVC(None, float, is_required=True, min=0., max=1.0e10, units='millimiters.seconds^-1', doc_str='Vertical swimming speed during diel vertical migrations'),
                                 ...
                                 'start': PVC(None, float, is_required=True, min=0., units='seconds', doc_str='Age of particles to start diel vertical migrations'),
                                 })


    def check_requirements(self):
        self.check_class_required_fields_prop_etc(requires3D=True, required_props_list=['velocity_modifier', 'light'])  

    def add_required_classes_and_settings(self):
        info = self.info
        # add particle prop to track light at particle positions
        si.add_class('particle_properties', class_name='ManuallyUpdatedParticleProperty', name='light', initial_value=0., description='Solar radiation currently experienced by particles in umol/m2/s-1')      
